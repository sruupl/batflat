...
class Admin extends Main
{
    ...
    public function drawTheme($file)
    {
        ...
        $this->assign['update_access'] = ($access == 'all')
            || in_array('settings', explode(',', $access)) ? true : false;
        ...
        if ($row && ($details = $this->getModuleInfo($name))) {
            if (($this->getUserInfo('access') == 'all')
                ...
                if (method_exists($this->module->{$name}, $method)) {
                    $details['content'] = call_user_func_array(
                        [$this->module->{$name}, $method],
                        array_values($params)
                    );
                } elseif (method_exists($this->module->{$name}, $anyMethod)) {
                    $details['content'] = call_user_func_array(
                        [$this->module->{$name}, $anyMethod],
                        array_values($params)
                    );
                } else {
    ...
    public function login($username, $password, $remember_me = false)
    {
        ...
        if ((time() - $attempt['expires']) < 0) {
            $this->setNotify(
                'failure',
                sprintf(
                    $this->lang['general']['login_attempts'],
                    ceil(($attempt['expires'] - time()) / 60)
                )
            );
            return false;
        }
        ...
        if (count($row) && password_verify(trim($password), $row['password'])) {
            ...
            $this->db('login_attempts')
                ->where('ip', $_SERVER['REMOTE_ADDR'])->save(['attempts' => $attempt['attempts'] + 1]);
            $attempt['attempts'] += 1;
            ...
            if ($attempt['attempts'] % 3 == 0) {
                $this->db('login_attempts')
                    ...
                    ->save(['expires' => strtotime("+10 minutes")]);
