...
class Site extends SiteModule
{
    public function init()
    {
        ...
        if (count($slug) == 3 && $slug[0] == 'blog' && $slug[1] == 'post') {
            $row = $this->db('blog')->where('status', '>=', 1)
                ->where('published_at', '<=', time())
                ->where('slug', $slug[2])->oneArray();
            if ($row) {
    ...
    public function routes()
    {
        ...
        $this->route('blog/(:int)', 'importAllPosts');
        $this->route('blog/post/(:str)', 'importPost');
        $this->route('blog/tag/(:str)', 'importTagPosts');
        $this->route('blog/tag/(:str)/(:int)', 'importTagPosts');
        $this->route('blog/feed/(:str)', 'generateRSS');
    }
    ...
    public function getLatestPosts()
    {
        ...
        $rows = $this->db('blog')
            ...
            ->limit($limit)
            ->select([
                'blog.id', 'blog.title', 'blog.slug', 'blog.intro',
                'blog.content', 'users.username', 'users.fullname'
            ])
            ->toArray();
    ...
    public function importPost($slug = null)
    {
        ...
        if (!empty($slug)) {
            ...
            if (!empty($row)) {
                ...
                $row['author']['name'] = !empty($row['author']['fullname'])
                    ? $row['author']['fullname'] : $row['author']['username'];
                ...
                if ($this->core->loginCheck()) {
                    if ($assign['published_at'] > time()) {
                        $assign['content'] = '<div class="alert alert-warning">'
                            . $this->lang('post_time') . '</div>' . $assign['content'];
                    }
                    if ($assign['status'] == 0) {
                        $assign['content'] = '<div class="alert alert-warning">'
                            . $this->lang('post_draft') . '</div>' . $assign['content'];
                    }
                }

                // date formatting
                $assign['published_at'] = (new \DateTime(date("YmdHis", $assign['published_at'])))
                    ->format($this->settings('blog.dateformat'));
                ...
                $this->tpl->set(
                    ...
                    [
                        ...
                        // Same code on line 191
                        'desc' => trim(
                            mb_strimwidth(
                                htmlspecialchars(
                                    strip_tags(
                                        preg_replace('/\{(.*?)\}/', null, $assign['content'])
                                    )
                                ),
                                0,
                                155,
        ...
        $this->core->append('<link rel="alternate" type="application/rss+xml" title="RSS" href="'
            . url(['blog', 'feed', $row['lang']]) . '">', 'header');
        ...
        $this->core->append('<meta property="og:description" content="' . trim(
            mb_strimwidth(htmlspecialchars(strip_tags(
                preg_replace('/\{(.*?)\}/', null, $assign['content'])
            )), 0, 155, "...", "utf-8")
        ) . '">', 'header');
        if (!empty($row['cover_photo'])) {
            $this->core->append('<meta property="og:image" content="'
                . url(UPLOADS . '/blog/' . $row['cover_photo']) . '?' . $row['published_at'] . '">', 'header');
        }
     ...
     */
    public function importAllPosts($page = 1)
    {
        $page = max($page, 1);
        ...
        foreach ($rows as $row) {
            ...
            // date formatting
            $row['published_at'] = (new \DateTime(date("YmdHis", $row['published_at'])))
                ->format($this->settings('blog.dateformat'));
            $keys = array_keys($this->core->lang['blog']);
            $vals = array_values($this->core->lang['blog']);
        ...
        $count = $this->db('blog')->where('status', 2)
            ->where('published_at', '<=', time())
            ->where('lang', $_SESSION['lang'])
            ->count();
        ...
        $this->core->append('<link rel="alternate" type="application/rss+xml" title="RSS" href="'
            . url(['blog', 'feed', $_SESSION['lang']]) . '">', 'header');
     ...
     * get array with all posts
     */
    public function importTagPosts($slug, $page = 1)
    {
        $page = max($page, 1);
        $perpage = $this->settings('blog.perpage');
        ...
        $count = $this->db('blog')->leftJoin('blog_tags_relationship', 'blog_tags_relationship.blog_id = blog.id')
            ->where('status', 2)
            ->where('lang', $_SESSION['lang'])
            ->where('published_at', '<=', time())
            ->where('blog_tags_relationship.tag_id', $tag['id'])
