...
class Site extends SiteModule
{
    ...
    public function getSearch($phrase, $index = 1)
    {
        ...
        if (strlen($phrase) < $phraseMinLength) {
            $page['content'] = sprintf($this->lang('too_short_phrase'), $phraseMinLength);
                ...
                // select pages
            $pages = $this->db()->pdo()
                ->prepare("SELECT * FROM pages WHERE lang = ? AND (title LIKE ? OR content LIKE ?)");
            ...
            foreach ($pagesArray as &$item) {
                ...
                // select blog entries
            $blog = $this->db()->pdo()
                ->prepare("SELECT * FROM blog WHERE lang = ? AND status = ? AND (title LIKE ? OR content LIKE ?)");
    ...
    private function insertSearchBox()
    {
        return $this->draw('input.html');
    }

    private function insertResults(array $results)
    {
